...
'''(Dirich---Levenberg-Marquardt (LM) optimizer-------------------------'''
# parameters counter
    ...
    try:
        ...
        while (step <= tr_iter_max):
            ...
            if (Comput_old == True):  # need to compute loss_old and J_old
                ...
                print(X_o.shape,F_o.shape,X_b.shape,F_b.shape)
            ...
            # 在 train_PINNs_LM 函数中，更新 best_l2 时（原代码已存在）
            if l2_loss < best_l2:
                best_l2 = l2_loss.item()
                best_epoch = step
                # --- 修正：先同步 func_params 到 model，再保存 ---
                with torch.no_grad():
                    p_idx = 0
                    for param in model.parameters():
                        if param.requires_grad:
                            param.copy_(func_params[p_idx])
                            p_idx += 1
                # torch.save(model.state_dict(), 'best_model_rbf.mdl')  # 现在保存的是训练后的参数
