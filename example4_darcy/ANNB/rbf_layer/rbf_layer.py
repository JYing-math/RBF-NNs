...
class RBFLayer(nn.Module):
    ...
    y(x) = sum_{i=1}^N a_i * prod^d_j=1 phi(eps^j_i * (x-c_i^j))
    ...
    ----------
        ...
        normalization: False for coding reason
    ...
    def _make_parameters(self) -> None:
        ...
        if self.constant_shape_parameter:
            ...
            # change the shape parameter from 1d array to 2d one.
            self.log_shapes = nn.Parameter(
    ...
    def reset(self,
        ...
        if self.initial_centers_parameter is None:
            ...
            self.kernels_centers = nn.Parameter(
                self.initial_centers_parameter, requires_grad=True)
        ...
        if self.initial_shape_parameter is None:
            ...
            self.log_shapes = nn.Parameter(
                self.initial_shape_parameter, requires_grad=True)
        ...
        if self.initial_weights_parameters is None:
            ...
            self.weights = nn.Parameter(
                self.initial_weights_parameters, requires_grad=True)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # c and diff has size B x num_kernels x Fin
        ...
        # Apply parameter, eps_r has size B x num_kernels x Fin
        eps_r = self.log_shapes.exp().expand(batch_size, self.num_kernels, self.in_features_dim) * diff
